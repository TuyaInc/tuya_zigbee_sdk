...
#define ECODE_NVM3_ERR_NO_VALID_PAGES               (ECODE_EMDRV_NVM3_BASE | 0x00000003U)        ///< Initialisation aborted, no valid page found
...
#define NVM3_MAX_OBJECT_SIZE_X          (NVM3_MAX_OBJECT_SIZE + 8)      // Adjust for object header
#define FLASH_PAGE_SIZE_X               (FLASH_PAGE_SIZE - 20)          // Adjust for page header
...
typedef struct nvm3_Cache {
  ...
  size_t            entryCount;
  bool              overflow;
  size_t            statMaxContent;
  uint32_t          statCntHit;
  uint32_t          statCntMiss;
} nvm3_Cache_t;
 ...
 *   Normally the application should not be conserned with the erase count value.
 *   But if NVM3 is substituting a previous solution, it is possible to transfer
 *   the erase count to NVM3 when initializing the NVM for the first time.
 *   The erase count must be set before the @ref nvm3_open is called, and it will
 ...
 *   erase count. After @ref nvm3_open has been called, the value will be consumed
 *   and have no effect on further calls to @ref nvm3_open.
 *
 * @param[in] eraseCnt
 *   The erase count.
 ******************************************************************************/
 ...
 *   and size, care must be taken. One can either move the start address up or
 *   down in memory and adjust the size accordingly to keep the end address,
 *   or keep address and just change the size.
 *   It is not possible to resize the area by doing changes in both ends of the
 *   NVM address range at the same time.
 *   If the resize operation return @ref ECODE_NVM3_OK, the instance is still
 ...
 * @{
   ...
   is needed. The @ref nvm3_repack() is used to do the actual repacks, and it
   will perform repacks only if the operation is needed.
   ...
   A NVM3 function that deletes or modifies a data or counter object will trigger
   ...
   member of the @ref nvm3_Init_t struct used by the @ref nvm3_open() function.
   ...
   @note The repack threshold can be changed to avoid that multiple modifications
   of objects between user called repacks are causing automatic repacks. Care
   should be taken because "high" values of the repack headroom may cause
   ...
   to the objects, and the performance will very much depend on objects beeing
   ...
   The NVM3 library is using some global variables to store intermediate data
   ...
   but rather at the time the user application is built, the size of the data
   ...
   @n These functions work on all objects. @ref nvm3_enumObjects() provides a
   way to get a list of keys to valid objects in the NVM. The search can also
   ...
   @n Functions to write, read and increment 32-bit counter objects.
   ...
   The minimim required NVM size is dependent on both the NVM page size and the
   NVM3_MAX_OBJECT_SIZE value. For a device with 2 kB page size and some typical
   values for NVM3_MAX_OBJECT_SIZE will give the following minimum required
   number of pages:
   ...
   The NVM3 error handling is classic in the way that most of the functions
   returns an error code. The @ref nvm3_countObjects is the only function that
   differs from this sceme because it returns the actual number of objects
   found.

   The behaviour and return values from most of the functions like @ref
   nvm3_readData, @ref nvm3_writeData etc. should be pretty self explanatory,
   while the @ref nvm3_open is a bit different. @ref nvm3_open will always try
   to recover from the previous state and continue without any error if possible.
   The rationale behind this is that if a valid NVM3 instance has been
   ...
   error, that is an indication that there is either a design or coding error,
   or that many of the NVM pages have been marked as BAD leaving unsufficient
