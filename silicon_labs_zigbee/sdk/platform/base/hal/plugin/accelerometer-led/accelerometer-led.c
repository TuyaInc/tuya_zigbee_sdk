...
void halAccelerometerLedInitialize(void)
{
  ...
  NVIC_DisableIRQ(ACC_INT_INT_EN_BIT); // Clear ACC_INT top level int enable
  EVENT_GPIO->FLAG = ACC_INT_FLAG_BIT; // Clear stale ACC_INT interrupt
  EVENT_MISS->MISS = ACC_INT_MISS_BIT; // Clear stale missed ACC_INT interrupt
  // Configure ACC_INT
  ACC_INT_SEL(); // Point IRQ at the desired pin
  ACC_INT_INTCFG  = EVENT_GPIO_CFGx_MOD_DISABLED; // No filter
  ACC_INT_INTCFG |= EVENT_GPIO_CFGx_MOD_BOTH_EDGES; // 3 = both edges

  NVIC_EnableIRQ(ACC_INT_INT_EN_BIT); // Set top level interrupt enable
...
void ACC_INT_ISR(void)
{
  ...
  EVENT_MISS->MISS = ACC_INT_MISS_BIT; // Clear missed interrupt flag
  EVENT_GPIO->FLAG = ACC_INT_FLAG_BIT; // Clear top level interrupt flag
