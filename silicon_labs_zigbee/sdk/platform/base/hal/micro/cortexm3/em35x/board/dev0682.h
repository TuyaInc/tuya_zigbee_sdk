/** @file hal/micro/cortexm3/em35x/board/dev0682.h
...
/** @addtogroup board
 *  @brief Functions and definitions specific to dev0682 BoB with a PCB4601
 *  EM359x RCM
 *
 * The dev0682 BoB with PCB4601 EM359x RCM has the following example GPIO
 * configuration. This board file reflects this configuration.
 * - PA0 - USB Data-
 * - PA1 - USB Data+
 * - PA2 - USB Enumerated
 * - PA3 - USB V_bus sense
 ...
 * - PB6 - BUTTON0
 * - PB7 - PIEZO
 * - PC0 - JRST or TRACEDATA[1]
 * - PC1 - FLASH_ENABLE or TRACEDATA[3]
 * - PC2 - JTAG (JTDO) or SWO or TRACEDATA[0]
 * - PC3 - JTAG (JTDI) or TRACECLK
 * - PC4 - JTAG (JTMS) or SWDIO
 * - PC5 - FEM_CTX
 * - PC6 - BUTTON1 or OSC32B
 * - PC7 - TEMP_ENABLE or OSC32A
 * - PD0 - UNUSED
 * - PD1 - SC3 TXD
 * - PD2 - SC3 RXD
 * - PD3 - SC3 nCTS
 ...
 * abstraction on top of the LEDs for verbose coding.
 */
...
/** @name Flash serial controller selection
 *
 * Defining EXTERNAL_FLASH_SERIAL_CONTROLLER to an integer between 1 and 4
 * (inclusive) causes the Flash driver to use the corresponding serial
 * controller (if available) rather than the old default (usually SC2, SC1 for
 * some drivers if SPI_FLASH_SC1 is defined)
 */
//@{

/**
 * @brief The PCB4601 RCM has a W25Q80BVSN SPI Flash IC connected to SC4.
 */
#if !defined(EXTERNAL_FLASH_SERIAL_CONTROLLER) && !defined(SPI_FLASH_SC1)
  #define EXTERNAL_FLASH_SERIAL_CONTROLLER  4
#endif
//@} //END OF Flash serial controller selection

/** @name Flash serial rate selection
 *
 * Defining EXTERNAL_FLASH_RATE_LINEAR and EXTERNAL_FLASH_RATE_EXPONENTIAL
 * causes the Flash driver to use these values when setting the serial
 * controller rate registers. The default rate when these are not defined
 ...
 *             (LIN+1) * 2^EXP
 *
 * where PCLK (SYSCLK divided by two) is generally 12 or 6 MHz. Both
 * EXTERNAL_FLASH_RATE_LINEAR and EXTERNAL_FLASH_RATE_EXPONENTIAL must be
 * betwen 0 and 15 (inclusive).
 */
//@{

/**
 * @brief The PCB4601 RCM has a W25Q80BVSN SPI Flash IC, and therefore uses the
 * spiflash-class1.c driver to communicate with it. This driver defaults to 12
 * MHz operation, which is too fast for the combination of the EM359x and
 * onboard multiplexer. We must therefore reduce the speed to 6 MHz.
 */
#define EXTERNAL_FLASH_RATE_LINEAR        1
#define EXTERNAL_FLASH_RATE_EXPONENTIAL   0
//@} //END OF Flash serial rate selection

/** @name EEPROM_USES_SHUTDOWN_CONTROL
 *
 * When EEPROM_USES_SHUTDOWN_CONTROL is defined, logic is enabled in the EEPROM
 * driver which drives a line high upon initialization. In Ember reference
 * designs, the enable pin must be driven high in order to use the EEPROM. This
 * option is intended to be enabled when running app-bootloader on designs based
 * on current Ember reference designs. PB7 is selected by default unless
 * EEPROM_POWER_PORT_PIN is defined.
 */
//@{

/**
 * @brief This define does not equate to anything.  It is used as a
 * trigger to enable the logic that drives the line high in the EEPROM driver.
 * Default is to leave this logic disabled.
 */
//#define EEPROM_USES_SHUTDOWN_CONTROL

/**
 * @brief Define which allows configuration and toggling of the EEPROM enable.
 * This must be defined when defining EEPROM_USES_SHUTDOWN_CONTROL for the
 * shutdown control to properly work on this board.
 */
#ifdef EEPROM_USES_SHUTDOWN_CONTROL
  #define EEPROM_POWER_PORT_PIN PORTC_PIN(1)
#endif
...
#define DEFINE_GPIO_RADIO_POWER_BOARD_MASK_VARIABLE()                           \
  GpioMaskType gpioRadioPowerBoardMask = { (BIT32(PORTB_PIN(0)) /* FEM_CPS */   \
                                            | BIT32(PORTC_PIN(5)) /* FEM_CTX */ \
                                            ) }
...
#define DEFINE_POWERUP_GPIO_CFG_VARIABLES()                                    \
  uint16_t gpioCfgPowerUp[9] = {                                               \
    ...
    ((GPIOCFG_UNUSED     << PD0_CFG_BIT)                                       \
     | (GPIOCFG_OUT_ALT    << PD1_CFG_BIT)                       /* SC3TXD  */ \
     | (GPIOCFG_IN_PUD     << PD2_CFG_BIT)                       /* SC3RXD  */ \
     | (GPIOCFG_IN_PUD     << PD3_CFG_BIT)),                     /* SC3nCTS */ \
    ((GPIOCFG_OUT_ALT    << PD4_CFG_BIT)                         /* SC3nRTS */ \
     | (GPIOCFG_UNUSED     << PD5_CFG_BIT)                                     \
     | (GPIOCFG_UNUSED     << PD6_CFG_BIT)                                     \
     | (GPIOCFG_UNUSED     << PD7_CFG_BIT)),                                   \
    ((GPIOCFG_OUT_ALT    << PE0_CFG_BIT)                                       \
     | (GPIOCFG_IN_PUD     << PE1_CFG_BIT)                                     \
     | (GPIOCFG_OUT_ALT    << PE2_CFG_BIT)                                     \
     | (GPIOCFG_OUT        << PE3_CFG_BIT))                                    \
  }
...
#define DEFINE_POWERUP_GPIO_OUTPUT_DATA_VARIABLES()                        \
  uint8_t gpioOutPowerUp[5] = {                                            \
    ...
    ((0                  << PC0_BIT)                                       \
     ...
     (1                  << PC7_BIT)),                                     \
    ((GPIOOUT_UNUSED     << PD0_BIT)                                       \
     | (1                  << PD1_BIT)                       /* SC3TXD  */ \
     | (1                  << PD2_BIT)                       /* SC3RXD  */ \
     | (1                  << PD3_BIT)                       /* SC3nCTS */ \
     | (0                  << PD4_BIT)                       /* SC3nRTS */ \
     | (GPIOOUT_UNUSED     << PD5_BIT)                                     \
     | (GPIOOUT_UNUSED     << PD6_BIT)                                     \
     | (GPIOOUT_UNUSED     << PD7_BIT)),                                   \
    ((1                  << PE0_BIT)                                       \
     | (GPIOOUT_PULLUP     << PE1_BIT)                                     \
     | (0                  << PE2_BIT)                                     \
     | (1                  << PE3_BIT))                                    \
  }
...
#define DEFINE_POWERDOWN_GPIO_OUTPUT_DATA_VARIABLES()                        \
  uint8_t gpioOutPowerDown[5] = {                                            \
    ...
    ((GPIOOUT_UNUSED     << PD0_BIT)                                         \
     | (1                  << PD1_BIT)                         /* SC3TXD  */ \
     | (1                  << PD2_BIT)                         /* SC3RXD  */ \
     | (GPIOOUT_PULLDOWN   << PD3_BIT)                         /* SC3nCTS */ \
     | (0                  << PD4_BIT)                         /* SC3nRTS */ \
     | (GPIOOUT_UNUSED     << PD5_BIT)                                       \
     | (GPIOOUT_UNUSED     << PD6_BIT)                                       \
     | (GPIOOUT_UNUSED     << PD7_BIT)),                                     \
...
#define SET_POWERUP_GPIO_OUTPUT_DATA_REGISTERS() \
  ...
  GPIO_PDOUT = gpioOutPowerUp[3];                \
  GPIO_PEOUT = gpioOutPowerUp[4];
...
#define SET_POWERDOWN_GPIO_OUTPUT_DATA_REGISTERS() \
  ...
  GPIO_PDOUT = gpioOutPowerDown[3];                \
  GPIO_PEOUT = gpioOutPowerDown[4];
