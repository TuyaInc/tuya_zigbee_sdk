/** @file hal/host/generic/compiler/platform-common.h
 * See @ref platform_common and micro specific modules for documentation.
 *
 ...
 * all platforms.  Some definitions can be overriden by the specific
 * PLATFORM_HEADER for your platform.
 *
 ...
 * parameter.
 */
int8_t halCommonMemPGMCompare(const void *source0, void PGM *source1, uint16_t bytes);

/**
 * @brief Same as the C stdlib memcpy(), but handles copying from const
 * program space.
 */
void halCommonMemPGMCopy(void* dest, void PGM *source, uint16_t bytes);
