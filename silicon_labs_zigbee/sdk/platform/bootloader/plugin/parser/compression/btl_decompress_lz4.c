...
int32_t gbl_lz4ReadMemory(size_t backtrackOffset, uint8_t *data, size_t length)
{
  ...
  if (backtrackOffset > lz4ParserContext.outputOffset) {
...
#ifdef PARSER_TEST
    (void) memcpy(data,
                  &parserBuffer[programmingAddress - parser_getApplicationAddress()],
                  copyLength);
#else
    (void) memcpy(data,
    ...
    uint32_t appPcAddress = parser_getApplicationAddress() + 4UL;
    ...
    uint32_t upgradePcAddress = parser_getBootloaderUpgradeAddress() + 4UL;
