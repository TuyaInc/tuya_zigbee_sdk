...
def fimware_info_get():
    ...
    oem_flag = fileJson['fimwareInfo']['oem_flag']
    file.close()
...
def generate_fimware_macro_config_h(fileObj):
    ...
    fileObj.writelines("#define OEM_FLAG".ljust(TABLE_SPACE)  + oem_flag + '\n')
...
def ato_generate_config_mk(*args,**kwargs):
    ...
    fileObject = open("firmware_ver_temp_file", 'w')
    fileObject.writelines(version)
    fileObject = open("firmware_name_temp_file", 'w')
...
def ato_generate_callback_c():
    ...
    if os.path.exists(callback_file_source_path):
        if os.path.isfile(callback_file_source_path):
            if os.path.exists("src/" + name + "_callback.c"):
